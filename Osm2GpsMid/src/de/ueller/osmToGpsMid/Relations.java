...
//			System.out.println("check relation " + r + "is valid()=" + r.isValid() );
			if (r.isValid()) {
				...
				if (processValidRealtion(r, removeWays, stats)) {
					i.remove();
				}
			} else { // ! r.isValid()
				...
				if (conf.useHouseNumbers && ("associatedStreet".equals(r.getAttribute("type"))
					...
					stats.houseNumberRelationIgnoreCount++;
				}
			}
		}
		...
		System.out.println("info: accepted " + stats.houseNumberCount + " housenumber-to-street connections from associatedStreet relations");
		System.out.println("info: ignored " + stats.houseNumberRelationIgnoreCount + " associatedStreet (housenumber) relations");
		System.out.println("info: ignored " + stats.houseNumberInterpolationIgnoreCount + " associatedStreet (housenumber) relation interpolation ways");
		System.out.println("info: processed " + stats.houseNumberRelationAcceptCount + " associatedStreet (housenumber) relations"
			+ ", of which " +  stats.houseNumberRelationProblemCount + " had problems");
		System.out.println("info: ignored " + stats.boundaryIgnore + " boundary=administrative multipolygon relations");
	}

	/**
	 * @param r
	 */
	private boolean processValidRealtion(Relation r, List<Way> removeWays, Stats stats) {
		...
		if (tagType != null && conf.isRelationExpanded(tagType + "=" + tagValue)
			...
			for (Long ref : r.getWayIds()) {
				...
				if (combinedRelationExpanded && w.containsKey(key)) {
					...
					if (w2 != null) {
						...
						for (String t : r.getTags()) {
							if (w2.containsKey(t)) {
								...
								for (String s : w2.getAttribute(t).split(";")) {
									if (s.equals(r.getAttribute(t))) {
										exists = true;
										break;
									}
								}
					...
					long newId = FakeIdGenerator.makeFakeId();
		...
		} else if (conf.useHouseNumbers && (
				"associatedStreet".equals(tagType) || "street".equals(tagType)
				)) {
			...
			for (Long ref : r.getWayIds(Member.ROLE_STREET)) {
				...
				for (Long wayref : r.getWayIds(Member.ROLE_HOUSE)) {
					...
					if (housew.containsKey("addr:interpolation")) {
						...
						stats.houseNumberInterpolationIgnoreCount++;
						...
						stats.houseNumberRelationProblemCount++;
			...
			stats.houseNumberRelationAcceptCount++;
			result = true;
		} else if ("multipolygon".equals(tagType)) {
...
//			}
				...
				if (w == null) {
					System.out.println("Way " + ref + " was not found but referred as outline in ");
...
//				//System.out.println("Adding way " + w.toUrl() + " as OUTER");
				...
				// for relation; so it's conceivable
			...
			for (Long ref : r.getWayIds(Member.ROLE_INNER)) {
				Way w = wayHashMap.get(ref);
				if (w == null) {
					System.out.println("Way " + ref + " was not found but referred as INNER in ");
					System.out.println("  relation "+ r.toUrl() + " I'll ignore this relation");
					return false;
				}
			...
			try {
				...
				// neither method below to remove type tag works
				//r.getTags().remove("type");
	...
	private static final class Stats {
		public int houseNumberCount = 0;
		public int houseNumberInterpolationIgnoreCount = 0;
		public int houseNumberRelationAcceptCount = 0;
		public int houseNumberRelationProblemCount = 0;
		public int houseNumberRelationIgnoreCount = 0;
		public int boundaryIgnore = 0;
